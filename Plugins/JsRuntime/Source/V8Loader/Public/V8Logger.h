...
DECLARE_LOG_CATEGORY_EXTERN(LogJsV8, Log, All);
...
public:
    ...
    static void LogTryCatch(v8::Isolate* Isolate,
        v8::TryCatch& TryCatch,
        const TCHAR* Prefix = TEXT("V8"))
    {
        ...
        if (Message.IsEmpty())
        {
            UE_LOG(LogJsV8, Error, TEXT("%s exception: %s"), Prefix, *ExceptionStr);
            return;
        }
        ...
        UE_LOG(LogJsV8, Error, TEXT("%s: %s"), Prefix, *MessageStr);
        UE_LOG(LogJsV8, Error, TEXT("%s: exception=%s"), Prefix, *ExceptionStr);
        UE_LOG(LogJsV8, Error, TEXT("%s: at %s:%d:%d"), Prefix, *ResourceName, LineNumber, StartColumn);
        ...
        if (SourceLine.ToLocal(&SourceLineLocal))
        {
            ...
            UE_LOG(LogJsV8, Error, TEXT("%s: %s"), Prefix, *SourceLineStr);
        }
        ...
        if (TryCatch.StackTrace(Context).ToLocal(&Stack) && Stack->IsString())
        {
            ...
            UE_LOG(LogJsV8, Error, TEXT("%s stack:\n%s"), Prefix, *StackStr);
        }
        else
        {
            UE_LOG(LogJsV8, Error, TEXT("%s: <no stack trace>"), Prefix);
        }
    }
}